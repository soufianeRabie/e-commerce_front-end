...
function ProductCardDetails() {
  const { dispatch, user, products } = useAuth()
  ...
  const handleAddToBasket = async (e) => {
    ...
    try {
      await Library.addToBasket(user, params.id, dispatch)
      Library.showToast('product added successfully')
    } catch ({ response }) {
      Library.showToast(response.data.message, 'error', 'top-right', 2000)
    }
  }
  return (
    <>
      <div className={'w-full flex  justify-center absolute mt-32 '}>
        ...
        <div
          className={
            'relative product m-2 sm:m-10 p-4 flex w-full max-w-lg flex-col overflow-hidden rounded-lg border border-gray-100 bg-white shadow-md hover:text-white hover:bg-gray-900 '
          }
        >
          ...
          <div className={'w-full'}>
            ...
            <div
              className={
                'flex w-3/4 m-5 mx-auto h-56 justify-center imgProductDetails'
              }
            >
              <img
